...
def ensure_cleaned():
    """Run data_prep.py if cleaned CSVs don't exist yet."""
    if not os.path.exists(LKA_CSV):
        subprocess.run([sys.executable, os.path.join(BASE_DIR, "data_prep.py")], check=True)
...
indicators = sorted(lka_df["Indicator Name"].unique())
all_countries = sorted(all_df["Country Name"].unique())
...
with st.sidebar:
    ...
    page = st.radio(
        "Go to",
        ["🏠 Overview", "📈 Trend Analysis", "🔬 Bivariate Analysis",
         "🌍 Global Comparison", "📊 Statistical Summary", "📋 Raw Data"],
    ...
    all_years = sorted(lka_df["Year"].unique())
...
# ─────────────────────────────────────────────────────────────────────────────
# Helper: filter LKA by selection
# ─────────────────────────────────────────────────────────────────────────────
...
if page == "🏠 Overview":
    st.markdown(
        ...
        <div class='hero-banner'>
            ...
            <div class='hero-sub'>
                Interactive exploration of World Bank Development Indicators &nbsp;·&nbsp;
                Data spans 1960 – 2025
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    if not selected_indicators:
        st.info("👈 Select at least one indicator from the sidebar to see KPIs.")
    else:
        ...
        if not df_ind.empty:
            ...
            pct_change   = ((latest_val - earliest_val) / abs(earliest_val)) * 100 if earliest_val else 0
            ...
            cards = [
                (col1, fmt(latest_val), f"Latest ({latest_year})",
                 f"{'▲' if pct_change >= 0 else '▼'} {abs(pct_change):.1f}% vs {earliest_year}",
                 "up" if pct_change >= 0 else "down"),
                (col2, fmt(avg_val), "Period Average", f"Yrs {year_range[0]}–{year_range[1]}", ""),
                (col3, fmt(peak_val), f"Peak Value", f"Recorded in {peak_year}", "up"),
                (col4, str(len(df_ind)), "Data Points", f"Across {year_range[1]-year_range[0]+1} yrs", ""),
            ]
            ...
            fig.update_layout(**PLOT_LAYOUT, title_font_size=14)
            ...
            st.markdown(
                ...
                <div class='insight-box'>
                    📌 <strong>Key Insight:</strong> The <em>{ind}</em> shows an overall
                    <strong>{trend_word} trend</strong> of <strong>{abs(pct_change):.1f}%</strong>
                    between {earliest_year} and {latest_year}. The peak value of
                    <strong>{fmt(peak_val)}</strong> was recorded in <strong>{peak_year}</strong>.
                    The period average stands at <strong>{fmt(avg_val)}</strong>.
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    with col_c:
        st.metric("Total LKA Records", len(lka_df))
    ...
    st.markdown(
        ...
        <div class='insight-box'>
            ...
            Source: <strong>World Bank – World Development Indicators</strong>. The dataset covers
            Sri Lanka (country code <code>LKA</code>) across <strong>{len(indicators)}</strong>
            sustainability-related indicators spanning from <strong>{year_min}</strong> to
            <strong>{year_max}</strong>. Use the sidebar to select indicators and filter by year
            range to explore detailed analyses in the other pages.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
# ─── TREND ANALYSIS ───────────────────────────────────────────────────────────
    ...
    st.markdown("<div class='section-header'>📈 Trend Analysis — Sri Lanka</div>", unsafe_allow_html=True)

    if not selected_indicators:
        st.warning("Please select at least one indicator from the sidebar.")
        st.stop()

    tab1, tab2, tab3 = st.tabs(["📉 Line / Area", "📊 Bar Chart", "🔢 Year-over-Year Change"])
    ...
    with tab1:
        ...
        if df_multi.empty:
            ...
            viz_type = st.radio("Chart Type", ["Line", "Area"], horizontal=True, key="line_area")
            fig = go.Figure()
            for i, ind in enumerate(selected_indicators):
                ...
                if viz_type == "Area":
                    fig.add_trace(go.Scatter(
                        x=sub["Year"], y=sub["Value"], name=ind,
            ...
            if len(selected_indicators) == 1:
                ...
                if show_ma:
                    ...
                    fig2.add_trace(go.Scatter(
                        x=sub["Year"], y=sub["Value"], name="Actual",
                        mode="lines+markers",
                        line=dict(color=COLORS[0], width=2),
                        marker=dict(size=4),
                    ))
                    fig2.add_trace(go.Scatter(
                        x=sub["Year"], y=sub["MA5"], name="5-Yr MA",
                        mode="lines",
    ...
    with tab2:
        ...
        if sub.empty:
            ...
            fig = px.bar(
                sub, x="Year", y="Value",
                color="Value",
                color_continuous_scale=[[0, "#1a365d"], [0.5, "#63b3ed"], [1, "#76e4f7"]],
                title=f"{ind_sel} — Annual Values",
                labels={"Value": ind_sel, "Year": "Year"},
            )
    ...
    with tab3:
        ...
        if len(sub) < 2:
            ...
            with col_left:
                fig = px.bar(
                    sub, x="Year", y="YoY Change",
                    color="YoY Change",
                    color_continuous_scale=[[0, "#fc8181"], [0.5, "#f6ad55"], [1, "#68d391"]],
            ...
            # Stats table
            st.markdown("<div class='section-header'>📋 YoY Change Statistics</div>", unsafe_allow_html=True)
            stats = sub[["Year", "Value", "YoY Change", "YoY % Change"]].copy()
            stats = stats.rename(columns={
                "Value": "Value", "YoY Change": "Δ (Abs)", "YoY % Change": "Δ (%)"
            })
            stats["Δ (Abs)"] = stats["Δ (Abs)"].round(3)
            stats["Δ (%)"]   = stats["Δ (%)"].round(2)
            st.dataframe(stats.set_index("Year"), use_container_width=True)


# ─── BIVARIATE ANALYSIS ───────────────────────────────────────────────────────
elif page == "🔬 Bivariate Analysis":
    st.markdown("<div class='section-header'>🔬 Bivariate & Multivariate Analysis</div>",
                unsafe_allow_html=True)

    if len(indicators) < 2:
        st.info("Need at least 2 indicators for bivariate analysis.")
        st.stop()

    tab1, tab2, tab3 = st.tabs(["🔵 Scatter / Correlation", "🌡 Heatmap", "📦 Distribution"])

    # ── Tab 1: Scatter ──
    with tab1:
        col_x, col_y = st.columns(2)
        with col_x:
            x_ind = st.selectbox("X-axis Indicator", indicators, index=0, key="biv_x")
        with col_y:
            y_ind = st.selectbox("Y-axis Indicator", indicators,
                                  index=min(1, len(indicators)-1), key="biv_y")

        x_df = lka_df[(lka_df["Indicator Name"] == x_ind) &
                       (lka_df["Year"] >= year_range[0]) &
                       (lka_df["Year"] <= year_range[1])][["Year", "Value"]].rename(columns={"Value": "X"})
        y_df = lka_df[(lka_df["Indicator Name"] == y_ind) &
                       (lka_df["Year"] >= year_range[0]) &
                       (lka_df["Year"] <= year_range[1])][["Year", "Value"]].rename(columns={"Value": "Y"})
        ...
        if merged.empty:
            ...
            st.markdown(
                f"<div class='insight-box'>📐 <strong>Pearson Correlation:</strong> "
                f"<strong style='color:{'#68d391' if corr > 0 else '#fc8181'}'>{corr:.4f}</strong> "
                f"— {'Strong positive' if corr > 0.7 else 'Moderate positive' if corr > 0.3 else 'Strong negative' if corr < -0.7 else 'Weak'} relationship</div>",
                unsafe_allow_html=True,
            )
    ...
    with tab2:
        ...
        heat_inds = st.multiselect(
            ...
            indicators,
            default=indicators[:min(6, len(indicators))],
            key="heat_inds",
        )
        if len(heat_inds) < 2:
            ...
            if pivot.shape[1] < 2:
                ...
                corr_mat = pivot.corr()
                # Shorten labels
                short = {c: (c[:25] + "…" if len(c) > 25 else c) for c in corr_mat.columns}
                corr_mat = corr_mat.rename(index=short, columns=short)
                fig = px.imshow(
                    corr_mat,
                    color_continuous_scale=[[0, "#fc8181"], [0.5, "#2d3748"], [1, "#68d391"]],
                    zmin=-1, zmax=1,
                    text_auto=".2f",
                ...
                fig.update_layout(
                    **PLOT_LAYOUT,
                    ...
                    coloraxis_colorbar=dict(
                        title="r", tickfont=dict(color="white"),
    ...
    # ── Tab 3: Distribution ──
    with tab3:
        dist_ind = st.selectbox("Indicator for distribution", selected_indicators or indicators[:1], key="dist_ind")
        sub = lka_df[
        ...
        if sub.empty:
            ...
            with col_r:
                ...
                fig2.add_trace(go.Box(
                    y=sub, name=dist_ind[:30], boxpoints="all",
                    marker_color=COLORS[1], line_color=COLORS[0],
                    pointpos=-1.8, jitter=0.4,
                ))
                fig2.update_layout(**PLOT_LAYOUT, title="Box Plot", yaxis_title=dist_ind)
                st.plotly_chart(fig2, use_container_width=True)
...
# ─── GLOBAL COMPARISON ───────────────────────────────────────────────────────
    ...
    if comp_df.empty:
        st.info("No data for the selected indicator / countries.")
    else:
        tab1, tab2, tab3 = st.tabs(["📉 Time Series", "📊 Bar Race (Latest)", "📋 Ranking Table"])

        with tab1:
            fig = px.line(
                comp_df, x="Year", y="Value", color="Country Name",
        ...
        with tab2:
            ...
            latest_bar = comp_df[comp_df["Year"] == latest_year_avail].sort_values("Value", ascending=True)
            fig = px.bar(
                latest_bar, x="Value", y="Country Name", orientation="h",
                color="Country Name", color_discrete_sequence=COLORS,
                title=f"{comp_ind} — Latest Year ({latest_year_avail})",
                labels={"Value": comp_ind},
            )
            fig.update_layout(**PLOT_LAYOUT, showlegend=False, yaxis_title="")
            # Highlight Sri Lanka bar
            for trace in fig.data:
                if trace.name == "Sri Lanka":
                    trace.marker.color = "#f6ad55"
            st.plotly_chart(fig, use_container_width=True)

        with tab3:
            pivot_tab = (
                comp_df.pivot_table(index="Country Name", columns="Year", values="Value")
                .round(2)
            )
            pivot_tab.columns = [str(c) for c in pivot_tab.columns]
            st.dataframe(pivot_tab.style.background_gradient(cmap="Blues", axis=None),
...
# ─── STATISTICAL SUMMARY ─────────────────────────────────────────────────────
    ...
    # Build summary table for all selected indicators
    rows = []
    ...
    if not rows:
        ...
        st.dataframe(
            summary_df.style
                ...
                .format("{:.3f}", subset=["Min", "Max", "Mean", "Median",
                                          "Std Dev", "25th %ile", "75th %ile",
                                          "Skewness", "Kurtosis"]),
            use_container_width=True,
        )

        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("<div class='section-header'>📈 Violin / Distribution Comparison</div>",
                    unsafe_allow_html=True)
        ...
        for i, ind in enumerate(selected_indicators):
            ...
            fig.add_trace(go.Violin(
                ...
                fillcolor=COLORS[i % len(COLORS)].replace("rgb", "rgba").replace(")", ",0.25)"),
            ))
        fig.update_layout(
            **PLOT_LAYOUT,
            title="Value Distribution by Indicator",
            yaxis_title="Value",
            violingap=0.1, violingroupgap=0.05,
        )
        st.plotly_chart(fig, use_container_width=True)
...
# ─── RAW DATA ────────────────────────────────────────────────────────────────
    ...
    raw_show = st.radio("Dataset", ["Sri Lanka Only", "All Countries"], horizontal=True)
    source_df = get_filtered() if raw_show == "Sri Lanka Only" else all_df[
        (all_df["Year"] >= year_range[0]) & (all_df["Year"] <= year_range[1])
    ]

    # Search
    search_q = st.text_input("🔍 Filter rows (any column)", "")
    if search_q:
        mask = source_df.apply(lambda col: col.astype(str).str.contains(search_q, case=False)).any(axis=1)
...
st.markdown(
    ...
    <div style='text-align:center; font-size:0.75rem; color:rgba(255,255,255,0.3); padding:0.5rem;'>
        Sri Lanka Sustainability Dashboard &nbsp;·&nbsp;
